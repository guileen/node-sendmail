...
function DKIMSign (email, options) {
  ...
  var headers = match && email.substr(0, match.index) || ''

  var body = match && email.substr(match.index + match[0].length) || email
  ...
  var canonicalizedHeaderData = DKIMCanonicalizer.relaxedHeaders(headers, options.headerFieldNames || defaultFieldNames)

  var canonicalizedDKIMHeader = DKIMCanonicalizer.relaxedHeaderLine(dkim)

  var signer; var signature
